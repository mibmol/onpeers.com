...
export const ChevronDown: FC<ISvgProps> = ({
	className = '',
	sizes = 'w-6 h-6',
	stroke = 'currentColor',
	strokeWidth = 2,
...
export const TWSpinner: FC<{
	color?: string;
	size?: string;
	animating?: boolean;
}> = memo(({ color = 'text-blue-500', size = 'w-8 h-8', animating = false }) => {
	return (
		<svg
			className={`${animating && 'spin-fast'} ${size} ${color}`}
			fill="none"
			viewBox="0 0 24 24"
		>
			<circle
